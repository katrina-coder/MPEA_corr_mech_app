...
class AlloyProblem(Problem):
    def __init__(self, objectives, generator, regressors, classifiers,
        ...
        #   +N for each required element (ALL must be present, not just one)
        #   +1 (max single-element fraction ≤ 60%)
        n_constr = 1 + 1  # max_elements + max_fraction
        if banned_indices:  n_constr += len(banned_indices)
        if required_indices: n_constr += len(required_indices)  # one constraint per required element
        ...
        self.required_indices = required_indices or []  # element indices that ALL must be present
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        # Normalise compositions to sum=1 — all fraction-based constraints use this
        comp32 = alloys39[:, :32].copy()
        rs32   = comp32.sum(axis=1, keepdims=True); rs32[rs32 == 0] = 1
        comp32_norm = comp32 / rs32

        # G1: max elements — n_elements - max_elements <= 0
        n_elements = (comp32_norm > 0.005).sum(axis=1).astype(float)
        G = [(n_elements - self.max_elements)]

        # G2: banned elements — normalised fraction must be <= 0.005
        for idx in self.banned_indices:
            G.append(comp32_norm[:, idx] - 0.005)

        # G3: ALL required elements must each be present (> 0.005 normalised fraction)
        #     One constraint per required element: -comp_norm[:, idx] + 0.005 <= 0
        for idx in self.required_indices:
            G.append(-comp32_norm[:, idx] + 0.005)

        # G4: no single element > 60% of total composition (prevents SPEAs)
        G.append(comp32_norm.max(axis=1) - 0.60)
...
def decode_results(res_X, generator, comp_min, comp_max, regressors,
    ...
    for comp in alloys39[:, :32]:
        # comp is already normalised (alloys39[:,:32] /= rs above)
        # Re-normalise elements above threshold to guarantee exact sum=1 in display
        active = {j: comp[j] for j in range(32) if comp[j] > 0.005}
        total  = sum(active.values())
        norm   = {j: v/total for j, v in active.items()} if total > 0 else active
        parts  = [f"{ELEMENTS[j]}{norm[j]:.3f}" for j in norm]
        names.append("".join(parts))
        n_elements_list.append(len(parts))
        al_fractions.append(round(norm.get(1, 0.0), 4))   # Al is index 1
        comp_sums.append(round(sum(norm.values()), 4))     # should always be 1.0
    ...
    return pd.DataFrame({
        ...
        'Composition sum':        comp_sums,       # sanity check — always 1.0
    })
...
with st.sidebar:
    ...
    required_elements = st.multiselect(
        ...
        default=[],
        help="Optional. Every element selected here MUST appear (> 0.005 mol fraction) "
             "in every optimised alloy — e.g. select Al and Fe to guarantee both are present. "
             "Leave empty to impose no requirement — the optimiser freely chooses from the allowed pool.")
