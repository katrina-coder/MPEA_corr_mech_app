"""
step2_retrain_models_A.py  —  Pipeline A: Separate Models  (v4)
──────────────────────────────────────────────────────────────
...
print(f"scikit-learn {sklearn.__version__}  |  Pipeline A v4\n")
...
MECH_FEATURES = ELEM_COLS + PROCESS_COLS + EMP_COLS + PHASE_COLS           # 58-dim
CORR_FEATURES = ELEM_COLS + PROCESS_COLS + EMP_COLS + PHASE_COLS + ELECTROLYTES + ['conc_norm']  # 66-dim
...
def train_reg(name, sub, target, feats, log_scale=False, fname=None):
    ...
    # 5-fold CV for reported R²
    mean_r2, std_r2 = cv_r2(X, y)

    # Final model on ALL observed rows
    rf = RandomForestRegressor(n_estimators=100, max_depth=50,
                               random_state=0, n_jobs=-1)
    rf.fit(X, y)
